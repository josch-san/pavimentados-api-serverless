...
class TestTaskModelFeatures:
    def test_instance_video_input_task(self):
        ...
        task = Task.parse_obj({**form, 'UserId': mocks.USER_ID})
        task.initialize_inputs(inputs, mocks.BUCKET_NAME)

        assert task.Inputs.VideoFile.Content.Key.startswith(f'pavimenta2/user:{mocks.USER_ID}/road_sections_inference/{task.Id}/inputs/VideoFile_')
        assert task.Inputs.GpsFile.Content.Key.startswith(f'pavimenta2/user:{mocks.USER_ID}/road_sections_inference/{task.Id}/inputs/GpsFile_')
    ...
    def test_instance_image_input_task(self):
        ...
        assert task.Inputs.ImageBundle.Content[0].Key.startswith(f'pavimenta2/user:{mocks.USER_ID}/road_sections_inference/{task.Id}/inputs/ImageBundle_')
        assert task.Inputs.GpsFile.Content.Key.startswith(f'pavimenta2/user:{mocks.USER_ID}/road_sections_inference/{task.Id}/inputs/GpsFile_')
